...
void dev_print (block_dev_desc_t *dev_desc)
{
	...
	case IF_TYPE_USB:
		...
		break;
	case IF_TYPE_DOC:
		printf("device type DOC\r\n");
		return;
	case IF_TYPE_NAND:
		printf("Please desc FTL version or something\r\n");
		return;
	       ...
	       block_dev_desc_t **dev_desc)
{
	...
	hwpart_str = strchr((s8 *)dev_hwpart_str, '.');
	if (hwpart_str) {
	...
	if (hwpart_str) {
		hwpart = strtoul(hwpart_str, &ep, 16);
		if (*ep) {
...
//		dev_part_str = getenv("bootdevice");
	...
	part_str = strchr((s8 *)dev_part_str, ':');
	if (part_str) {
	...
	if (!part_str || !*part_str) {
		...
		part = (int)strtoul((s8 *)part_str, &ep, 16);
		/*
