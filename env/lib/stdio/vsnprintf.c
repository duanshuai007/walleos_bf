...
int vsnprintf_old(char * buf, size_t n, const char * fmt, va_list ap)
{
    const char * p = fmt;
    char ch;
    //add by duanshuai
...
is_unsigned: switch (rank)
             {
                 ...
                 case rank_longlong:
                     ...
                     u32 new_ap = ap - 8;   //获取原ap的地址，再经过va_arg之后，ap已经加上了对应的增量
                     //printk("ap=%08x,start=%08x\r\n", ap-8, start);
                     //printk("%08x %08x %08x %08x\r\n%08x %08x %08x %08x\r\n", 
                     //        readl(start), readl(start+4), readl(start+8), readl(start+12), 
                     //        readl(start+16), readl(start+20), readl(start+24), readl(start+28));
                     if((new_ap - start) % 8)  //当前参数与堆栈参数开始的位置相差不是8的倍数
                     {
                         ap -= 4;               //当前位置不正确，将ap向前移动4个字节，然后重新取参数
                         val = (uintmax_t)va_arg(ap, unsigned long long);
                     }
                     break;
             }
