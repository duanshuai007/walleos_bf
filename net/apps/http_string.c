...
*/
int str_index_of(char *a, char *b)
{
...
//	clrdst = (char*)realloc(clrdst, strlen(clrdst) + 1);
	char *tmpClrdst = (char *)malloc(strlen(clrdst) + 1);
	memset(tmpClrdst, 0, strlen(clrdst) + 1);
	memcpy(tmpClrdst, clrdst, strlen(clrdst));
...
char* base64_encode(char *clrstr) 
{
	...
	char *tmpB64dst = (char *)malloc(strlen(b64dst) + 1);
	memset(tmpB64dst, 0, strlen(b64dst) + 1);
	memcpy(tmpB64dst, b64dst, strlen(b64dst));
