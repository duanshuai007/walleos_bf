...
static bool_t console_getcode(struct rl_buf * rl, u32_t * code)
{
	...
	if (s5p4418_serial_read(rl->ch, (u8_t *)&c, 1) != 1)
		return FALSE;
	...
	case TTY_STATE_CSI:
		if(c >= '0' && c <= '9')
		{
			if((unsigned int)rl->num_params < ARRAY_SIZE(rl->params))
			{
...
static bool_t console_putcode(int ch, u32_t code)
{
	...
	s5p4418_serial_write_string(ch, buf);
...
static void console_gotoxy(struct rl_buf * rl, int x, int y)
{
	...
	s5p4418_serial_write_string(rl->ch, buf);
...
static void rl_delete_one_char(struct rl_buf * rl) 
{
    s32_t x, y;
    s32_t pos, w;
    u32_t i;
...
static void rl_insert(struct rl_buf * rl, u32_t * str)
{
	...
	{	
		p = malloc(sizeof(u32_t) * rl->size * 2);
		if(!p){
			return;
		}

		memset(p, 0x0, sizeof(u32_t) * rl->size * 2);
		memcpy(p, rl->buf, rl->size);
		if (rl->buf != NULL)
		{
			free(rl->buf);
		}
...
static bool_t readline_handle(struct rl_buf * rl, u32_t code)
{
    ...
    //printf("readline_handle: rl = %08x, code = %d\r\n", rl, code);
	switch(code)
	{
	case 0x0:	/* null */
		break;
